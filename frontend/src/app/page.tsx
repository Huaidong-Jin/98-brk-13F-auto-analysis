...
import { ChangesLeaderboard } from "@/components/charts/ChangesLeaderboard";
...
export default function HomePage() {
  ...
  const [changesSummary, setChangesSummary] = useState<
    { quarter: string; new_count: number; increased_count: number; decreased_count: number; closed_count: number }[] | null
  >(null);
  ...
  const [latestTopHoldings, setLatestTopHoldings] = useState<
    { issuer_name: string; cusip: string; value_usd: number; weight_pct: number; rank: number }[] | null
  >(null);
  ...
  const [artifacts, setArtifacts] = useState<
    { artifact_type: string; quarter: string; generated_at: string | null; validation_status: string; download_url: string }[]
  >([]);
  ...
  const totalB = data?.total_value_usd != null ? (data.total_value_usd / 1e9).toFixed(1) : "—";
  ...
  const qoqDeltaPct =
    data && typeof (data as any).qoq_delta_pct === "number"
      ? `${(data as any).qoq_delta_pct >= 0 ? "+" : ""}${(data as any).qoq_delta_pct.toFixed(1)}%`
      : "—";
  const top1Weight =
  ...
  const top1DeltaPct =
    data && typeof (data as any).top1_delta_pct === "number"
      ? `${(data as any).top1_delta_pct >= 0 ? "+" : ""}${(data as any).top1_delta_pct.toFixed(1)}%`
      : "—";
  const top1DeltaUsd =
    ...
    <main className="space-y-8">
      ...
      {mode === "analyst" && data?.narrative && (
        <section className="rounded-lg border border-zinc-200 bg-zinc-50 p-4 dark:border-zinc-800 dark:bg-zinc-900/50">
          <h2 className="text-subtitle text-ink-primary mb-2">{t("home.atAGlance")}</h2>
      ...
      {data?.portfolio_trend && data.portfolio_trend.length > 0 && (() => {
        ...
        const valuesB = trend.map((d) => d.total_value_usd / 1e9);
        const maxB = Math.max(...valuesB, 1);
        const yMax = Math.ceil(maxB / 50) * 50 || 100;
        const chartHeightPx = 256;
        const n = trend.length;
        const step = n <= 12 ? 1 : n <= 24 ? 2 : Math.max(1, Math.floor(n / 8));
        ...
        return (
          ...
          >
            <div className="flex gap-1 items-end" style={{ height: chartHeightPx }}>
              <div className="flex flex-col justify-between h-full text-caption text-ink-tertiary pr-2 border-r border-grid shrink-0 w-16">
                {yTicks.map((v, idx) => (
                  <span key={idx}>{`$${v.toFixed(0)}B`}</span>
                ))}
              </div>
              <div className="relative flex-1 h-full">
                ...
                <div className="relative flex items-end gap-px min-w-0 h-full">
                  {trend.map((d) => {
                    const b = d.total_value_usd / 1e9;
                    const barHeightPx = Math.max(2, (b / yMax) * chartHeightPx);
                    return (
                      <UnifiedTooltip
                        key={d.quarter}
                        quarter={d.quarter}
                        value={formatUSD(d.total_value_usd)}
                        className="relative flex-1 min-w-0 shrink-0 flex items-end"
                      >
                        <div
                          className="w-full bg-accent rounded-t opacity-90 hover:opacity-100 transition-opacity"
            ...
            <div className="flex gap-1 mt-1">
              ...
              <div className="flex-1 flex gap-px min-w-0 text-caption text-ink-tertiary">
                {trend.map((d, i) => (
                  <div key={d.quarter} className="flex-1 min-w-0 shrink-0 text-center truncate" style={{ fontSize: 10 }}>
                    {labelIndices.has(i) ? d.quarter : ""}
                  </div>
                ))}
              </div>
            </div>
      ...
      {data?.portfolio_trend && data.portfolio_trend.length >= 2 && (() => {
        const trend = data.portfolio_trend;
              ...
              {last20.map((g) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`w-full shrink-0 ${isPos ? "bg-positive rounded-t" : "bg-negative rounded-b"}`}
                      style={{ height: barH }}
                    />
      ...
      {data?.concentration_trend && data.concentration_trend.length > 0 && (() => {
        ...
        const stepC = nc <= 8 ? 1 : Math.max(1, Math.floor(nc / 5));
        const labelIndicesC = new Set<number>([0, nc - 1]);
        for (let i = stepC; i < nc - 1; i += stepC) labelIndicesC.add(i);
        const totalsByQuarter = new Map<string, number>(
          (data?.portfolio_trend ?? []).map((d) => [d.quarter, d.total_value_usd]),
        );
        return (
          <ChartFrame
          ...
          >
            <div className="flex items-end gap-px" style={{ height: hPx }}>
              {conc.map((d) => {
                const h1 = Math.round((d.top1_pct / 100) * hPx);
                const h2 = Math.round(((d.top5_pct - d.top1_pct) / 100) * hPx);
                const h3 = Math.round(((d.top10_pct - d.top5_pct) / 100) * hPx);
                ...
                return (
                  <UnifiedTooltip
                    key={d.quarter}
                    quarter={d.quarter}
                    value={`Top1: ${formatPct(d.top1_pct)} (${top1ValueUsd}) · Top5: ${formatPct(
                      d.top5_pct,
                    )} · Top10: ${formatPct(d.top10_pct)} (${top10ValueUsd})`}
                    className="relative flex-1 min-w-0 shrink-0 flex flex-col justify-end"
                  >
                    <div className="bg-chart-1 w-full shrink-0" style={{ height: h1 }} />
                    <div className="bg-chart-2 w-full shrink-0" style={{ height: h2 }} />
                    <div className="bg-chart-3 w-full shrink-0" style={{ height: h3 }} />
                  </UnifiedTooltip>
                );
              })}
            </div>
            <div className="flex gap-px mt-1 text-caption text-ink-tertiary">
              {conc.map((d, i) => (
                <div key={d.quarter} className="flex-1 min-w-0 shrink-0 text-center truncate" style={{ fontSize: 10 }}>
                  {labelIndicesC.has(i) ? d.quarter : ""}
                </div>
              ))}
            </div>
            {latestTopHoldings && latestTopHoldings.length > 0 && (
              <div className="mt-4 text-caption text-ink-secondary">
                <p className="mb-2">
                  {t("charts.concentrationLegendTitle", { quarter: data?.latest_quarter ?? "—" })}
                </p>
                <ol className="space-y-1">
                  {latestTopHoldings.slice(0, 10).map((h) => (
                    <li key={h.cusip} className="flex items-center justify-between gap-2">
      ...
      {mode === "analyst" && changesSummary && changesSummary.length > 0 && (() => {
        const last12 = changesSummary.slice(-12);
        const totals = last12.map(
          (c) => c.new_count + c.increased_count + c.decreased_count + c.closed_count
        );
                ...
                {netSeries.map((n) => {
                  ...
                  return (
                    <UnifiedTooltip
                      key={n.quarter}
                      quarter={n.quarter}
                      value={`${n.net >= 0 ? "+" : ""}${n.net} = (New + Increased) − (Decreased + Closed)`}
                      className="relative flex-1 min-w-0 shrink-0 flex flex-col"
                    >
                      <div style={{ height: topSpacer, minHeight: 0 }} />
                      <div
                        className={`w-full shrink-0 ${
                          isPos ? "bg-positive rounded-t" : "bg-negative rounded-b"
                        }`}
                        style={{ height: barH }}
                      />
                      <div style={{ height: bottomSpacer, minHeight: 0 }} />
              ...
              <div className="flex items-center justify-between mt-2 text-caption text-ink-tertiary">
                ...
                <div className="flex items-center gap-4 text-ink-secondary">
                  <div className="flex items-center gap-1">
                    ...
                    <span>
                      {t("quarter.changesNew")} / {t("quarter.changesIncreased")}
                    </span>
                  </div>
                  <div className="flex items-center gap-1">
                    <span className="inline-block w-3 h-3 rounded-sm bg-negative" />
                    <span>
                      {t("quarter.changesDecreased")} / {t("quarter.changesClosed")}
                    </span>
                  </div>
                </div>
              </div>
      ...
      {data?.latest_quarter && fullHoldings && (
        <section className="space-y-2">
          <div className="flex items-center justify-between">
            <h2 className="text-subtitle text-ink-primary">{t("explore.title")}</h2>
            <button
              type="button"
              className="text-caption text-accent hover:underline"
              onClick={() => setShowExplore((v) => !v)}
            >
              {showExplore ? t("explore.toggleCollapse") : t("explore.toggleExpand")}
            </button>
          </div>
          {showExplore && (
            <HoldingsTable
      ...
      {artifacts.length > 0 && (
        <section className="space-y-2">
          <h2 className="text-subtitle text-ink-primary">{t("download.title")}</h2>
          <p className="text-body text-ink-secondary">{t("download.intro")}</p>
            ...
            <table className="min-w-full text-left text-caption">
              <thead className="bg-zinc-50 dark:bg-zinc-900/60 text-ink-secondary">
                <tr>
                  ...
                  <th className="px-3 py-2 text-right">{t("download.download")}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800">
                {artifacts.slice(0, 8).map((a, idx) => (
                  <tr key={`${a.quarter}-${a.artifact_type}-${idx}`}>
                    <td className="px-3 py-1.5 text-ink-primary">{a.artifact_type}</td>
                    <td className="px-3 py-1.5 text-ink-secondary">{a.quarter}</td>
          ...
          <div className="space-y-1 text-caption text-ink-secondary">
            <div>
              ...
              {health.last_ingest_at && (
                <span className="ml-3">
                  Last ingest: {health.last_ingest_at} ({health.last_ingest_status ?? "unknown"})
                </span>
              )}
            </div>
            {latestMeta && (
      ...
      {topHoldingsTs && topHoldingsTs.series.length > 0 && topHoldingsTs.quarters.length > 0 && (() => {
        const { quarters, series } = topHoldingsTs;
        const hPx = 220;
        const yTicks = [100, 50, 0];
        // Top1 uses accent; Top2–Top5 use neutral chart colors
        const fillClasses = ["bg-accent", "bg-chart-2", "bg-chart-3", "bg-chart-4", "bg-chart-5"];
          ...
          >
                ...
                <div className="relative flex items-end gap-px h-full">
                  {quarters.map((q, qi) => (
                    <UnifiedTooltip
                      ...
                      quarter={q}
                      value={series
                        .map((s) => `${s.issuer_name}: ${formatPct(s.weights[qi] ?? 0)}`)
                        .join(" · ")}
                      className="relative flex-1 min-w-0 shrink-0 flex flex-col justify-end"
                    >
                      {series.map((s, si) => {
                        ...
                        return (
                          <div
                            key={s.cusip}
                            className={`${fillClasses[si] ?? "bg-chart-1"} w-full shrink-0`}
            ...
            <div className="flex items-start justify-between mt-2 gap-4">
              ...
              <div className="flex flex-wrap gap-3 text-caption text-ink-secondary">
                {series.map((s, idx) => (
                  <div key={s.cusip} className="flex items-center gap-1 max-w-[14rem]">
                    <span
